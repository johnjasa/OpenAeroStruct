    ...
    def compute_partials(self, inputs, partials):
        ...
        for surface in surfaces:
            ...
            if surface['symmetry']:
                ...
                # r1 = inputs[vectors_name][:, -1:, 1:  , :]
                # r2 = inputs[vectors_name][:, -1:, 0:-1, :]
                # derivs[3, :, -1:, :ny-1, :] += compute_finite_vortex_deriv1(r1, r2, trailing_array)[:, :, :ny-1, :, :]
                # derivs[1, :, -1:, :ny-1, :] += compute_finite_vortex_deriv2(r1, r2, trailing_array)[:, :, :ny-1, :, :]
                # # derivs[3, :, -1:, :ny-1, :] -= compute_semi_infinite_vortex_deriv(u, r1, trailing_array)[:, :, :ny-1, :, :]
                # # derivs[1, :, -1:, :ny-1, :] += compute_semi_infinite_vortex_deriv(u, r2, trailing_array)[:, :, :ny-1, :, :]
                # derivs[3, :, -1:, ny-1:, :][:, :, ::-1] += compute_finite_vortex_deriv1(r1, r2, trailing_array)[:, :, ny-1:, :, :]
                # derivs[1, :, -1:, ny-1:, :][:, :, ::-1] += compute_finite_vortex_deriv2(r1, r2, trailing_array)[:, :, ny-1:, :, :]
                # # derivs[3, :, -1:, ny-1:, :][:, :, ::-1] -= compute_semi_infinite_vortex_deriv(u, r1, trailing_array)[:, :, ny-1:, :, :]
                # # derivs[1, :, -1:, ny-1:, :][:, :, ::-1] += compute_semi_infinite_vortex_deriv(u, r2, trailing_array)[:, :, ny-1:, :, :]
                #
